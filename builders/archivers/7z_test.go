...
func TestBuild7z_Empty(t *testing.T) {
	got := archivers.Build7z(domain.Archive{}, domain.Group{})
	if got != "" {
		t.Errorf("Expected empty; got: %s", got)
	}
}

func TestBuild7z_GoldenCases(t *testing.T) {
	level := 9
	...
	tests := []struct {
		...
		},
		{
			...
			archive: domain.Archive{
				Name:     "full",
				Cwd:      "/project path",
				Method:   &domain.Method{Level: &level},
				Files:    []string{"file 1.txt", "file2.txt"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := archivers.Build7z(tt.archive, tt.group)
			goldenPath := filepath.Join("testdata", "7z-"+tt.name+".golden")
